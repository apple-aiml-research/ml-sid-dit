...
def fid10k_full(opts):
    fid = fid_and_clip.compute_fid_and_clip(opts, max_real=None, num_gen=10000,batch_size=8,compute_clip=False)
    return dict(fid10k_full=fid,open_clipscore_30k=float('nan'),clipscore30k=float('nan'))


@register_metric
def fid_clip_10k_full(opts):
    fid,open_clip_score,clip_score = fid_and_clip.compute_fid_and_clip(opts, max_real=None, num_gen=10000,batch_size=8,compute_clip=True)
    return dict(fid10k_full=fid,open_clipscore_10k=open_clip_score,clipscore30k=clip_score)
