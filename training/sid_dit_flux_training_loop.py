...
from training.sid_dit_flux_util import load_dit, sid_dit_generate, sid_dit_denoise, encode_prompt    
...
from training.precompute_latents import (
    ...
    process_and_save_latents_with_pipeline_multigpu_flux,
)
...
import warnings
warnings.filterwarnings("ignore", message="Passing `txt_ids` 3d torch.Tensor is deprecated.")
warnings.filterwarnings("ignore", message="Passing `txt_ids` 3d torch.Tensor is deprecated. Please remove the batch dimension and pass it as a 2d torch Tensor")
...
def training_loop(
    ...
    #with temporary_model_on_device(text_encoders, device):
        with torch.autocast(device_type="cuda",dtype=dtype_autocast,enabled=use_autocast):
            uncond_embeds, uncond_pooled_embeds,_ = encode_prompt(
                    text_encoders,
                    tokenizers,
                    negative_prompt,
        ...
        if use_fsdp or get_rank() == 0:
            ...
            #with temporary_model_on_device(text_encoders, rank_device):
                #with temporary_model_on_device(vae, rank_device):
                    with torch.no_grad():
                        with torch.autocast(device_type="cuda",dtype=dtype_autocast,enabled=use_autocast):
                            dist.print0(f"Grid z type: {type(grid_z)}, Grid c type: {type(grid_c)}")
                            if len(grid_z) > 0:
                                dist.print0(f"Grid z[0] shape: {grid_z[0].shape}, dtype: {grid_z[0].dtype}, device: {grid_z[0].device}")
                            if len(grid_c) > 0:
                                dist.print0(f"Grid c[0] type: {type(grid_c[0])}, length: {len(grid_c[0])}")
                                if len(grid_c[0]) > 0:
                                    dist.print0(f"Grid c[0][0] type: {type(grid_c[0][0])}")
                            dist.print0(G_fsdp.module.dtype,'G_fsdp dtype')
                    ...
                    with torch.autocast(device_type="cuda",dtype=dtype_autocast,enabled=use_autocast):
                        ...
                        with fsdp_sync(fake_score_fsdp, (round_idx == num_accumulation_rounds - 1)):
                            with torch.no_grad():
                                ...
                                #dist.print0(f"z shape: {z.shape}, dtype: {z.dtype}")
                                #dist.print0(f"noise shape: {noise.shape}, dtype: {noise.dtype}")
                                #dist.print0(f"contexts length: {len(contexts)}")
                                #dist.print0(f"init_timesteps shape: {init_timesteps.shape}, dtype: {init_timesteps.dtype}")
                                #if prompt_embeds is not None:
                                #    dist.print0(f"prompt_embeds shape: {prompt_embeds.shape}, dtype: {prompt_embeds.dtype}")
                                #if pooled_prompt_embeds is not None:
                                #    dist.print0(f"pooled_prompt_embeds shape: {pooled_prompt_embeds.shape}, dtype: {pooled_prompt_embeds.dtype}")
                            ...
                            # Print image input shape for debugging
                            #dist.print0(f"images shape: {images.shape}, dtype: {images.dtype}")
                            if train_diffusiongan:
                            ...
                            #dist.print0(output.shape,target.shape)
...
#                         G_fsdp.module.apply(lambda m: restore_bn_dropout(m, name=str(m)))
            ...
            for round_idx in range(num_accumulation_rounds):
                with torch.autocast(device_type="cuda",dtype=dtype_autocast,enabled=use_autocast):
                    ...
                    with fsdp_sync(G_fsdp,False):
                        ...
                        latents, t, guidance, prompt_embeds, pooled_prompt_embeds, text_ids, latent_image_ids,height,width,vae_scale_factor= sid_dit_generate(
                            dit=G_fsdp,
                            latents=z,
                            contexts=contexts,
                            init_timesteps=init_timesteps,
                    ...
                    with fsdp_sync(G_fsdp, (round_idx == num_accumulation_rounds - 1)):
                        ...
                        images = sid_dit_generate(
                            ...
                            use_sd3_shift=use_sd3_shift,
                            text_feature_dtype=text_feature_dtype,
                            guidance=guidance,text_ids=text_ids, latent_image_ids=latent_image_ids,height=height,width=width,vae_scale_factor=vae_scale_factor
                        )                                        
                        ...
                        if train_diffusiongan:
                            ...
                            bce_loss = torch.nn.BCEWithLogitsLoss(reduction='none')
                            loss_gan = bce_loss(y_D.clamp(-10, 10),y_D_labels) if use_autocast else bce_loss(y_D.to(dtype=torch.float32).clamp(-10, 10),y_D_labels).to(dtype=torch.float32)
        ...
        try:
            del z,noise,y_fake,y_real,loss,loss_gan,sigmas, weight_factor, scale_factor 
        except NameError:
            pass
        torch.cuda.empty_cache()
